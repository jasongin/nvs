...
Generally, the following steps should work with any CI system:
 ...
 2. In the installation script, clone NVS, then use [`nvs add`](ADD.md) and [`nvs use`](USE.md) commands to install the version of Node.js indicated by that environment variable.
...
before_install:
# Install NVS.
- git clone --depth 1 https://github.com/jasongin/nvs ~/.nvs
- . ~/.nvs/nvs.sh
- nvs --version
install:
# Install the selected version of Node.js using NVS.
