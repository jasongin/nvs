...
function getCurrentVersion() {
	...
	for (let i = 0; i < pathEntries.length && !version; i++) {
		...
		if (pathEntry.toLowerCase().startsWith(settings.home.toLowerCase())) {
			...
			version = NodeVersion.tryParse(versionString, true);
 ...
 *
 * @param version The version of node to add to the PATH, or 'default' to use the
 * 	default (linked) version if any, or null to remove any NVS-managed versions
 * 	from the PATH.
	...
	for (let i = 0; i < pathEntries.length; i++) {
		...
		if (pathEntry.toLowerCase().startsWith(settings.home.toLowerCase())) {
			...
			if (versionString === linkName) {
				...
				previousVersion = NodeVersion.tryParse(versionString, true);
			}
		} else if (isWindows && pathEntry.toLowerCase() === getSystemLinkPath().toLowerCase()) {
			previousVersion = linkName;
		} else {
		...
		if (previousVersion) {
			if (i === 0 && version &&
				...
				if (!isWindows && !previousVersion.path) {
 ...
 *
 * @param version The version of node to run with, or 'default' to use the
 * 	default (linked) version if any.
 */
