...
function link(version) {
    ...
    if (!binPath) {
        ...
        throw new Error('Specified version not found.' + os.EOL +
            'To add this version now: nvs add ' + versionString, 'ENOENT');
    }
    ...
    try {
        ...
        if (e.code !== 'ENOENT') {
            throw new Error('Failed to read link: ' + linkPath, e);
        }
    }
...
function unlink(version) {
    ...
    try {
        ...
        if (e.code !== 'ENOENT') {
            throw new Error('Failed to remove symbolic link: ', e);
        }
    }
...
function linkToProgramFiles(linkTarget) {
    ...
    try {
        ...
        if (e.code !== 'ENOENT') {
            throw new Error('Failed to access path: ', e);
        }
    }
...
function linkToWindowsProfilePath(link, linkPath, isSystem) {
    ...
    let pathParts = profilePath.split(nvsUse.pathSeparator).filter(part => part);
    ...
    if (saveChanges) {
        nvsWindowsEnv.setEnvironmentVariable(
            'PATH', pathParts.join(nvsUse.pathSeparator), isSystem);
    }
