 ...
 */
function outdated() {
	const semver = require('../deps/semver');
	const versions = getVersions()
		.map(v => {
			return getRemoteVersionsAsync(v.remoteName).then(rvl => {
				let upgrades = rvl.filter(rv => canUpgrade(v, rv));
				if (upgrades.length > 0) {
					upgrades = ' [upgrade available: ' + upgrades[0] + ']';
				} else {
					upgrades = '';
				}
				return v.toString({ marks: true, label: true }) + upgrades;
			});
		});
	return Promise.all(versions);

	function canUpgrade(v, rv) {
		return semver.neq(v.semanticVersion, rv.semanticVersion) && semver.satisfies(rv.semanticVersion, '^' + v.semanticVersion);
	}
}
