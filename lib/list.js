...
const githubReleasesRegex = /^https?:\/\/github.com\/([^/]+)\/([^/]+)(?:\/releases)?\/?(?:#(.*))?$/;
...
function find(filter, versions) {
	...
	if (uniqueArchs.length > 0) {
		filteredVersions = filteredVersions.filter(
			v => v.arch === (filter.arch || NodeVersion.defaultArch)
		);
	}
	...
	if (!remoteName) {
		...
		childNames.forEach(childName => {
			...
			if (stats.isDirectory() && childName !== "node_modules") {
				result = result.concat(
					getVersionsInternal(childName, semanticVersion, arch)
				);
			}
		});
		...
		childNames.forEach(childName => {
			...
			if (stats.isDirectory()) {
				result = result.concat(
					getVersionsInternal(remoteName, childName, arch)
				);
			}
		});
	} else if (!arch) {
		// Scan all directories under the semantic version directory, which might be architectures.
		let childNames = readdirIfExists(
			path.join(settings.home, remoteName, semanticVersion)
		);
		childNames.forEach(childName => {
			...
			if (stats.isDirectory()) {
				result = result.concat(
					getVersionsInternal(remoteName, semanticVersion, childName)
				);
			}
		});
	} else {
		// Check if a valid full version directory was found.
		let version = NodeVersion.tryParse(
			remoteName + "/" + semanticVersion + "/" + arch,
			true
		);
		if (version) {
			try {
				let versionProperties = JSON.parse(
					fs.readFileSync(
...
function getRemoteVersionsAsync(remoteName) {
	...
	if (!remoteUri) {
		return Promise.reject(
			new Error("No URI found in settings.json for remote: " + remoteName)
		);
	}
	...
	return asyncResult.then(remoteVersions => {
		return remoteVersions
			.map(version => {
				version.local = !!localVersions.find(
					v =>
						v.remoteName === remoteName &&
						v.semanticVersion === version.semanticVersion
				);
				version.current =
					currentVersion && NodeVersion.equal(currentVersion, version);
				version.default =
					defaultVersion && NodeVersion.equal(defaultVersion, version);
				return version;
			})
...
function getNodejsRemoteVersionsAsync(remoteName, remoteUri) {
	let remoteIndexUri =
		remoteUri + (remoteUri.endsWith("/") ? "" : "/") + "index.json";

	return new Promise((resolve, reject) => {
		request
			.get(remoteIndexUri, (err, res, body) => {
				if (res.statusCode === 200) {
					let remoteIndex;
					try {
						remoteIndex = JSON.parse(body);
					} catch (e) {
						reject(
							new Error("Failed to parse index: " + remoteIndexUri, e)
						);
						return;
					}
...
function nodeReleaseInfoToVersion(remoteName, remoteUri, release) {
	...
	if (
		!semanticVersion.startsWith("v") ||
		(semanticVersion.startsWith("v0") &&
			!/^v0.[7-9]|1[0-9]/.test(semanticVersion))
	) {
	...
	release.files.forEach(f => {
		...
		if (ext === ".msi") {
			...
			if (/^0\./.test(semanticVersion) && arch === "x64") {
				...
				uri =
					uri.substr(0, uri.lastIndexOf("/") + 1) +
					arch +
					"/" +
					binaryName +
					"-v" +
...
function getGithubRemoteVersionsAsync(remoteName, remoteUri) {
	...
	return new Promise((resolve, reject) => {
		...
		https
					...
					res.on("end", () => {
						if (res.statusCode === 200) {
							...
							try {
								...
								reject(
									new Error(
										"Failed to parse GitHub releases query result: " +
											remoteUri,
										e
									)
								);
							...
							let versions = releases
								.map(
									githubReleaseInfoToVersion.bind(
										null,
										remoteName,
										repo,
										filter
			...
			.on("error", e => {
				reject(
					new Error("Failed to query GitHub releases: " + remoteUri, e)
				);
			});
	});
}
...
function githubReleaseInfoToVersion(remoteName, repo, filter, release) {
	...
	let packages = release.assets
		.map(a => {
			let fileName = path
				.basename(a.browser_download_url || "")
				.toLowerCase();
			...
			let os =
				ext === ".msi" ? "win" : fileNameParts[fileNameParts.length - 2];
			if (!arch || !os) {
				return null;
			}
...
function getNetworkRemoteVersionsAsync(remoteName, pathPattern) {
	...
	if (versionTokenIndex < 0 || archTokenIndex < 0) {
		return Promise.reject(
			new Error(
				"Invalid network path for remote: " +
					remoteName +
					"; " +
					versionToken +
					" and " +
