...
function getRemoteVersionsAsync(remoteName) {
    ...
    if (path.isAbsolute(remoteUri)) {
        ...
        asyncResult = getOfficialRemoteVersionsAsync(remoteName, remoteUri);
    }
 ...
 */
function getOfficialRemoteVersionsAsync(remoteName, remoteUri) {
            ...
            if (res.statusCode === 200) {
                ...
                res.on('end', () => {
                    ...
                    let versions = releases
                        .map(githubReleaseInfoToVersion.bind(null, remoteName, repo))
                        .filter(v => v);
                    resolve(versions);
                });
            } else if (res.statusCode === 404) {
 ...
 */
function githubReleaseInfoToVersion(remoteName, repo, release) {
...
module.exports = {
    ...
    getOfficialRemoteVersionsAsync,
