...
function list(remoteName, semanticVersion, arch) {
    ...
    if (!remoteName) {
        ...
        childNames.forEach(childName => {
            var childPath = path.join(settings.home, remoteName, childName);
            var stats = fs.lstatSync(childPath);
            if (stats.isDirectory()) {
        ...
        childNames.forEach(childName => {
            var childPath = path.join(settings.home, remoteName, semanticVersion, childName);
            var stats = fs.lstatSync(childPath);
            if (stats.isDirectory()) {
        ...
        var versionString = remoteName + '/' + semanticVersion + '/' + arch;
        if (nvsVersion.versionRegex.test(versionString)) {
            var version = { remoteName, semanticVersion, arch };
...
function installAsync(version) {
    return resolveSemanticVersionAsync(version).then(version => {
        ...
        if (binPath) {
            ...
            var remoteUri = settings.remotes[version.remoteName];
...
function downloadAsync(version, remoteUri) {
    ...
    var zipFileUri = remoteUri + (remoteUri.endsWith('/') ? '' : '/') +
