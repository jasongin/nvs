...
let nvsEnv = require('./env');  // Non-const enables test mocking
...
function list(remoteName, semanticVersion, arch) {
    ...
    if (!remoteName) {
        ...
        childNames.forEach(childName => {
            ...
            let stats = fs.lstatSync(childPath);
            if (stats.isDirectory()) {
                output += list(remoteName, childName, arch);
            }
        });
    } else if (!arch) {
        ...
        childNames.forEach(childName => {
            ...
            let stats = fs.lstatSync(childPath);
            if (stats.isDirectory()) {
                output += list(remoteName, semanticVersion, childName);
            }
        });
    } else {
        ...
        if (nvsVersion.versionRegex.test(versionString)) {
            ...
            let binPath = nvsEnv.getVersionBinary(version);
            if (binPath) {
                let currentVersion = nvsEnv.getCurrentVersion();
                let linkedVersion = nvsEnv.getLinkedVersion();
        ...
        if (binPath) {
            ...
            return downloadAsync(version, remoteUri).then(() => {
                ...
                if (binPath) {
                    ...
                    throw new Error('Install failed - executable file not found.');
                }
            });
        }
    });
...
function downloadAsync(version, remoteUri) {
    let zipFileExt = (nvsEnv.isWindows ? '.zip' : '.tar.gz');
        ...
        try {
            ...
            client.get(zipFileUri, (res) => {
                if (res.statusCode === 200) {
                    ...
                    reject(new Error('File not available: ' + zipFileUri + os.EOL +
                        'HTTP response status: ' + res.statusCode));
                } else {
                    reject(new Error('Failed to download file: ' + zipFileUri + os.EOL +
                        'HTTP response status: ' + res.statusCode));
                }
            }).on('error', (e) => {
                reject(new Error(
...
function uninstall(version) {
    ...
    let versionDir = nvsEnv.getVersionDir(version);
