...
let nvsVersion = require('./version');
 ...
 */
function migrateGlobalModules(sourceVersion, targetVersion, verbose) {
    if (!targetVersion) {
    ...
    if (nvsVersion.equal(sourceVersion, targetVersion)) {
    ...
    fs.readdirSync(sourceDir).forEach(childName => {
        ...
        if (childStats.isDirectory()) {
            if (!childName.startsWith('@')) {
                migratePackage(sourceDir, targetDir, childName, targetVersion, verbose);
            } else {
...
function getGlobalModulesDir(version) {
    ...
    if (!binPath) {
        ...
        throw new Error('Version not found: ' + versionString, Error.ENOENT);
    }
...
function migratePackage(sourceDir, targetDir, packageName, targetVersion, verbose) {
