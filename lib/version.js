...
class NodeVersion {
	...
	static _standardArchName(arch) {
		switch (arch) {
			...
			case 'x64':
			case 'amd64':
			case 'arm64': // new Macs with Apple M1 Chip
				return 'x64';
			case 'ppc64':
	...
	*/
	...
	get defaultArch() {
		switch (process.platform) {
			case 'darwin':
				if (process.arch === 'x64' && os.version().includes('_ARM64_') && this.semanticVersion && Number(this.semanticVersion.split('.')[0]) >= 16) {
					return NodeVersion._standardArchName('arm64');
				}
				return NodeVersion.defaultArch;
			default:
				return NodeVersion.defaultArch;
		};
	}
