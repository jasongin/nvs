...
function parse(versionString, requireFull) {
    ...
    let version = {
        ...
        os,
        toString: function () {
            return this.remoteName + '/' + this.semanticVersion + '/' + this.arch;
        },
    };
 ...
 * an error.
 */
function tryParse(versionString, requireFull) {
    try {
        return parse(versionString, requireFull);
    } catch (e) {
        return null;
    }
}
