...
function doCommand(args) {
	...
	switch (args[0]) {
		...
		default:
			if (isJsFile(args[0])) {
				// A .js file was specified without any command.
				// Execute an implicit run command with auto-detected version.
				return require('./auto').findAutoVersionAsync().then(version => {
					return require('./use').run(version, options.concat(args));
				});
			} else if (isJsFile(args[1])) {
				// A .js file was specified after (presumably) a version specifier.
				...
				// No .js file was specified. If a version specifier can be parsed
