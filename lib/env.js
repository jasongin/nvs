...
function link(version) {
    ...
    let linkPath = path.join(settings.home, linkName);
...
/**
 * Replaces the beginning of a path with ~ or %APPDATA%,
 * for shorter easier-to-read display.
 */
function homePath(fullPath) {
    if (isWindows) {
        let userAppdata = process.env['APPDATA'];
        if (fullPath.toLowerCase().startsWith(userAppdata.toLowerCase())) {
            let postScriptFile = process.env['NVS_POSTSCRIPT'];
            let inPowerShell = (postScriptFile &&
                path.extname(postScriptFile).toUpperCase() === '.PS1');
            return (inPowerShell ? '$env:APPDATA' : '%APPDATA%') +
                fullPath.substr(userAppdata.length);
        }
    } else {
        let userHome = process.env['HOME'];
        if (fullPath.toLowerCase().startsWith(userHome.toLowerCase())) {
            return '~' + fullPath.substr(userHome.length);
        }
    }
