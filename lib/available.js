...
var remoteNames = Object.keys(settings.remotes)
...
function listAsync(remoteName) {
    var listremoteIndexAsync = (i, foundName, output) => {
        if (remoteNames[i]) {
...
function formatRemoteIndex(remoteName, remoteIndex) {
    ...
    if (Array.isArray(remoteIndex)) {
        ...
        var lines = formatAsColumns(remoteIndex.filter(item => {
...
function downloadIndexAsync(remoteName) {
    ...
    var remoteUri = settings.remotes[remoteName];
    if (!remoteUri) {
        return Promise.reject(new Error('No URI found in settings.json for remote: ' + remoteName));
    }

    var remoteIndexUri = remoteUri + (remoteUri.endsWith('/') ? '' : '/') + 'index.json';

    var client = remoteIndexUri.startsWith('https:') ? https : http;
